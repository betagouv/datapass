...
module DgfipValidationMethods
  ...
  def rgpd_general_agreement_validation
    ...
    errors.add(:additional_content, :invalid,
      message: "Vous devez attester que votre organisation déclarera à la DGFiP l'accomplissement des formalités en terme de protection des données")
  end
  ...
  def api_rial_access_validation
    return if additional_content.any? do |k, v|
      v && %w[recherche_local_identifiant recherche_locaux_parcelle_cadastre recherche_locaux_adresse].include?(k)
    end
  ...
  def production_form_validation
    ...
    unless additional_content&.fetch("autorite_homologation_nom", false)&.present?
      errors.add(:additional_content, :invalid,
        message: "Vous devez renseigner le nom de l’autorité d’homologation ou du signataire du questionnaire de sécurité avant de continuer")
    end
    unless additional_content&.fetch("autorite_homologation_fonction", false)&.present?
      errors.add(:additional_content, :invalid,
        message: "Vous devez renseigner la fonction de l’autorité d’homologation ou du signataire du questionnaire de sécurité avant de continuer")
    end
    date_regex = /^\d{4}-\d{2}-\d{2}$/
    unless date_regex.match?(additional_content&.fetch("date_homologation", ""))
      errors.add(:additional_content, :invalid,
        message: "Vous devez renseigner la date de début de l’homologation ou de signature du questionnaire de sécurité au format AAAA-MM-JJ avant de continuer")
    end
    unless date_regex.match?(additional_content&.fetch("date_fin_homologation", ""))
      errors.add(:additional_content, :invalid,
        message: "Vous devez renseigner la date de fin de l’homologation au format AAAA-MM-JJ avant de continuer")
    end
    unless documents.where(type: "Document::DecisionHomologation").present?
      errors.add(:documents, :invalid,
        message: "Vous devez joindre le document de décision d’homologation ou le questionnaire de sécurité avant de continuer")
    end

    # Volumétrie
    unless additional_content&.fetch("volumetrie_appels_par_minute", false)&.present?
      errors.add(:additional_content, :invalid,
        message: "Vous devez renseigner la limitation d’appels par minute avant de continuer")
    end
